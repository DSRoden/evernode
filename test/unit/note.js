...
describe('Note', function(){
  ...
  describe('.uploadmobile', function(){
    it('should upload b64 image', function(done){
  ...
  describe('.upload', function(){
    it('should upload an image', function(done){
      var file = fs.createReadStream(__dirname + '/../fixtures/test_image.png');
      Note.upload({token: 'tok'}, file, 'test_image.png', noteId, function(err, results){
        expect(err).to.be.null;
        done();
      });
    });
